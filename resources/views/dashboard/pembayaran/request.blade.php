...
@section('pembayaran')
    @foreach ($datarequest as $data)
        <div class="flex h-10 my-3">
            ...
            <h1 class=" py-2 px-4 text-xl font-bold mx-3"> {{ $data->paketwifi->harga }}</h1>
            <form action="/pembayaran/{{ $data->id }}" method="post">
                @csrf
                <input name="jenis" type="hidden" value="{{ $data->paketwifi->jenis }}">
                <input name="nilai" type="hidden" value="{{ $data->paketwifi->nilai }}">
                <input name="nis" type="hidden" value="{{ $data->user->nis }}">
                <button type="submit"
